...
do
    ...
    function a.b()
        ...
        function p.NewRoundFrame(r, u, v, x, B, C)
            ...
            local J = {}
            function J.SetRadius(L, M) UpdateSliceScale(M) end
            function J.SetType(L, M) u = M F.Image = getImageForType(M) F.SliceCenter = getSliceCenterForType(M) UpdateSliceScale(r) end
            UpdateSliceScale(r)
    ...
    function a.c()
        ...
        function b.New(d, e, f)
            local g = {Enabled = e.Enabled or false, Translations = e.Translations or {}, Prefix = e.Prefix or "loc:", DefaultLanguage = e.DefaultLanguage or "en"}
            f.Localization = g
            return g
        end
        return b
...
aa.ScreenGui = aa.Creator.New("ScreenGui", {Name = "WindUI", Parent = au, IgnoreGuiInset = true, ScreenInsets = "None"}, {
...
function aa.CreateWindow(ax, ay)
    ...
    if ay.KeySystem then
        aA = false
        local function loadKeysystem() a.load "m".new(ay, gethwid and gethwid() or ah.LocalPlayer.UserId, function(aE) aA = aE end) end
        local aE = (ay.Folder or "Temp") .. "/" .. (gethwid and gethwid() or ah.LocalPlayer.UserId) .. ".key"
        if ay.KeySystem.SaveKey and isfile(aE) then
            local aF = readfile(aE)
            local valid = false
            if ay.KeySystem.KeyValidator then valid = ay.KeySystem.KeyValidator(aF)
            elseif not ay.KeySystem.API then valid = (type(ay.KeySystem.Key) == "table") and table.find(ay.KeySystem.Key, aF) or tostring(ay.KeySystem.Key) == tostring(aF)
            end
            if valid then aA = true else loadKeysystem() end
        else
            loadKeysystem()
        end
    ...
    if ay.Acrylic then ar.init() end

    local p = a.load "U"
    local u = p.Init(aE, ar.WindUI, ar.WindUI.TooltipGui)
    u:OnChange(function(v) aE.CurrentTab = v end)
    aE.TabModule = p

    function aE.Tab(v, x)
        x.Parent = aE.UIElements.SideBar.Frame
        local tab = u.New(x, ar.WindUI.UIScale)
        tab.CurrentContent = tab.Content

        function tab:Section(config)
            local title = config.Title or "Section"
            local sectionFrame = ao.New("Frame", {
                Name = "Section",
                ...
                ao.New("UIListLayout", {Padding = UDim.new(0, 8), SortOrder = Enum.SortOrder.LayoutOrder}),
                ao.New("UIPadding", {PaddingLeft = UDim.new(0, 12), PaddingRight = UDim.new(0, 12), PaddingTop = UDim.new(0, 12), PaddingBottom = UDim.new(0, 12)})
            })
            ao.NewRoundFrame(12, "Squircle", {
                Size = UDim2.new(1, 0, 1, 0),
            ...
            local contentHolder = ao.New("Frame", {
                Name = "Content",
                Size = UDim2.new(1, 0, 0, 0),
                AutomaticSize = Enum.AutomaticSize.Y,
                BackgroundTransparency = 1,
                ...
                ao.New("UIListLayout", {Padding = UDim.new(0, 6), SortOrder = Enum.SortOrder.LayoutOrder}),
                ao.New("UIPadding", {PaddingTop = UDim.new(0, 16)})
            })
            local oldContent = tab.CurrentContent
            tab.CurrentContent = contentHolder
            local methods = {"Button","Toggle","Slider","Dropdown","Colorpicker","Keybind","Input","Label","Paragraph","Textbox"}
            local originals = {}
            for _, name in ipairs(methods) do
                if tab[name] then
                    originals[name] = tab[name]
                    tab[name] = function(self, cfg)
                        cfg.Parent = contentHolder
                        return originals[name](self, cfg)
                    end
                end
            end
            function tab:EndSection()
                tab.CurrentContent = oldContent
            end
            return sectionFrame
        end
